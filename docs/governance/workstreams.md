...
flowchart LR
    ...
    M9 --> M10["M10<br/>every barrier refuses<br/>v0.4.0"]:::wip
    M10 --> PP["Pilot project<br/>private, GitHub Team"]:::todo
...
| D25 | `doctor` G12 fails with "remove every bypass actor" when **no ruleset** applies to `main`: there is no actor to remove, and the action that fits is G1's, creating the ruleset (P6). Found in M9's Task 2 | Planned: M10f — G12 names the ruleset to create |
| D26 | The checklist asks for four required checks, but GitHub only offers a check it has already seen: `Test sheet and cycle` and `PR scope and review budget` run on pull requests only, so a fresh repository cannot select them. Unblocked by opening a first pull request. Found in M9's Task 2 | Planned: M10f — said in G4's action and in the skeleton README |
| D27 | A plan carried the author's home directory four times, once in a block meant to be copy-pasted: it names a folder no other machine has, and publishes the locale and the layout of the workstation it was written on. The leak review was a human `grep`, and `gitleaks` looks for secrets, not for a home directory. Found by the maintainer reviewing pull request #50 (M9's Task 3) | v0.3.2: rule **H1** in `nstack hygiene`, inside `nstack fitness`, which the skeleton's own workflow already runs — every generated project inherits it |
| D28 | `decider` is checked for being non-empty only, while the charter template writes `"@<github-handle>"` and C2's own message promises a GitHub handle (P6): a charter copied from the template and never filled in goes green, and so does a team where a decision needs one owner. Found reviewing how PDR-0004 lands, whose third rule reads the recipient rather than guessing it | v0.3.2: C2 and C7 require one person's handle, the rule defined once beside `OWNER` |
