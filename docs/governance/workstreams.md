...
| M10 | **Every barrier the framework announces refuses**: D29 to D33, D35, D25 and D26, and PDR-0005, through to v0.4.0, proved on `tool-library` before the pilot — [plan](plans/2026-09-19-v0.4.0-every-barrier-refuses.md) | **Done (2026-09-19)**: M10a to M10g merged; the review before the tag found D36 to D43, fixed by #67 and #68; v0.4.0 published, tag on `486be72`, verified on published artefacts; M10h paid the update on `tool-library` (#31 to #33) and replayed the probes — **8 of 8 did what v0.4.0 promises**, `doctor` compliant. Exit decision **fix first**: D44 to D47 in v0.4.1, before the pilot |
| v0.5.0 | **V1 cannot be walked around, and a repository nothing guards is served honestly**: D44 to D47 — the proof run from the base's tree, its verdict never hidden nor mis-worded, K1's message between two cycles — plus **PDR-0006** (guarded or unguarded, and the record of what nothing can refuse) and **ADR-0002's clarification** (two channels: the registry publishes, the forge distributes the same tag). The pilot's precondition | To plan (2026-09-20): M10's exit decision **fix first**, and the maintainer's choice of 2026-09-20 to ship the three together rather than in two releases |
