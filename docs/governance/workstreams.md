...
| v0.5.0 | **V1 cannot be walked around, and a repository nothing guards is served honestly**: D44 to D47 — the proof run from the base's tree, its verdict never hidden nor mis-worded, K1's message between two cycles — plus **PDR-0006** (guarded or unguarded, and the record of what nothing can refuse) and **ADR-0002's clarification** (two channels: the registry publishes, the forge distributes the same tag). The pilot's precondition | [Plan](plans/2026-09-20-v0.5.0-a-proof-that-cannot-be-rewritten.md) written (2026-09-20), five workstreams then the release: M10's exit decision **fix first**, and the maintainer's choice of 2026-09-20 to ship the three together rather than in two releases |
