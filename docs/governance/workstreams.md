...
| D58 | **A project receives its agent's machine state.** `.claude/` and `.mcp.json` appear in the project folder; the skeleton's `.gitignore` covers `.claude/skills/` and `.claude/settings.local.json`, so it already knows this agent, but neither of these. A `git add -A` commits them. The pilot hit it and wrote the ignore rule itself (its pull request #4). Found on 2026-09-21 | The skeleton's `.gitignore` ignores what an agent runtime leaves behind. Planned with **PDR-0010** |
