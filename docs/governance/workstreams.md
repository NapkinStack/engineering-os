...
flowchart LR
    ...
    M10 --> V50["v0.5.0<br/>V1 proved from the base<br/>a repository nothing guards"]:::done
    V50 --> V60["v0.6.0<br/>what is announced refuses<br/>the documents say the truth"]:::done
    V60 --> PP["Pilot project<br/>public, running"]:::wip
    V60 --> V62["v0.6.1 · v0.6.2<br/>release notes<br/>engine and project apart"]:::done
    V62 --> M11["M11<br/>ceremony follows<br/>consequence · v0.7.0"]:::wip
...
| ADR-0003 | [Adopt English as the repository language](../adr/0003-adopt-english-as-the-repository-language.md) | Accepted (2026-09-16), criterion observed at the v0.2.0 to v0.4.0 releases; **shrunk to the rule in force on 2026-09-24** (M11h) — the migration it decided lives in the M7 plan and in git |
...
| ADR-0004 | [Give agents their own GitHub identity, behind a human approval](../adr/0004-give-agents-their-own-github-identity.md) | Accepted (2026-09-16); applied to this repository (M8f, 2026-09-17), criterion observed in the pilot project; clarified 2026-09-19: who verified (D32), an approval covers what it read (D31); clarified 2026-09-20: two powers, never in the same hand (PDR-0008); who approves a module corrected 2026-09-24 |
...
| PDR-0003 | [Approve a change on evidence of its behaviour](../pdr/0003-approve-a-change-on-evidence-of-its-behaviour.md) | Accepted (2026-09-16), clarified: what changes a module; applied by M8, measured in the pilot project; **measured on the pilot, recorded 2026-09-24** — the rule unchanged |
...
| D49 | **T2 cannot see a verifier launched by the author's session.** It compares declared names with the change's authors and the sessions their trailers name; a verifier subagent with a fresh context, its own identifier and the brief alone passes — which is what ran on #31 and #33, disclosed in each pull request's risks. Found in M10h | With the verifier's own App, which ADR-0004 already schedules: from then the verdict is a check the ruleset requires from that App. Until then, declared in the pull request. **Measured on the pilot (2026-09-22)**: it refused a sheet at every module change, and the project answered by ADR — a delegated verifier signs under its own agent identifier, never the parent's — which is consistent with what T2 claims to be, a declaration checked against the history. Its five rounds on one deliverable are the first real figures on the question `05-workflow.md` §7 leaves open: **8 real defects found, 2 of which the author would have shipped**, 3 refusals to merge, 5 of the author's fixtures shown to prove nothing, 4 of the author's measurements disproved |
...
| PDR-0004 | [Ask the right question, to the right person, at the right moment](../pdr/0004-ask-the-right-question-to-the-right-person.md) | Accepted (2026-09-18): the direction only — stage derived from the checks, hats in playbooks, recipient read from the project. Its criterion is met on M9's log. **Built in part by M11**: M11c routes the criticality to the decider; **M11j** builds the first axis, the stage (PDR-0007) |
...
| D51 | **A document still holding its template's words passes every declarative check.** `nstack plan` reports *compliant* for a charter whose only `success_criteria` entry is the template's `<An observable result, and when it is observed>` (C2), and for a `ready` deliverable whose title and acceptance criteria are the template's (C4); `nstack manifests` reports *compliant* for a module whose responsibility is `TODO - describe this module's business capability in one sentence.` (M2). D28 fixed this shape for the `decider` field alone. Found by the internal audit of 2026-09-20, reproduced on a project generated from the working tree | **Done (2026-09-20)**: v0.6.0 **b** — a declaration still carrying a placeholder, or opening with TODO, is refused by C2, C4, C7 and M2; for a module, from its first file of code, so that a generated module stays green (D33) |
| D52 | **Nothing ever mentions the consumers of a deprecated module.** `02-modules.md` §6 promises *"a `deprecated` module that gains a consumer → Red"*, and *"a `maintenance` module whose contract changes → Red"*; the engine reads `lifecycle` for M3 (valid value) and M5 (removal date) only. Found by the same audit | **Done (2026-09-20)**: v0.6.0 **c** — B8 reports every consumer of a deprecated module with its removal date, and the `maintenance` row names V1, which already covers the versions that matter. Reported rather than refused: what is already declared is bounded by the removal date, and "gains a consumer" is a diff notion a whole-tree rule cannot read |
| D53 | **A contract change never runs the other side's checks.** `03-contracts.md` §5 promises *"contract tests run in the CI of both modules"* and that a change breaking a declared consumer is caught before merge. `module-checks.yml` runs the verbs for modules with a **changed file**, so a change under `contracts/` runs the contracts module's tests and nobody else's; V1 compares the documents, it runs neither side's suite. Found by the same audit | **Done (2026-09-20)**: v0.6.0 **d** — `nstack modules --with-contract-sides` adds the producer and the declared consumers of a touched version, read from the manifests; the module workflow asks for them, and the oracle keeps the wiring from being lost |
| D54 | **A Copier refusal is reported with its last line, which can be the least informative one.** `project._explain` keeps the last line of a multi-line git error, so `fatal: adding files failed` arrives without `error: <file>: can only add regular files, symbolic links or git-directories`, and the action offered — check `--source`, `--ref` and the network — cannot help (P6, the D50 family). Found running the oracle on the v0.6.0 plan's own pull request | **Done (2026-09-20)**: v0.6.0 **a2** — the refusal keeps every line git wrote, and when git refused something local the action is local too: commit or stash the template's working tree, or remove the file git names |
| D55 | **The action added by D54 over-claims.** A git `error:` line is read as a refusal of the template's working tree, and the action says to commit or stash it. Verifying v0.6.0, a read-only Copier cache produced `error: cannot open '…/FETCH_HEAD': Read-only file system` and the same action, which could not help. The detail line is right — that is D54's fix — the action guesses. Found on 2026-09-20, verifying the published v0.6.0 | **Done (2026-09-20)**: the action points at the line git wrote rather than guessing which local cause it was; the read-only cache has its own test beside the working tree's. Ships with the next version |
| D56 | **`nstack init` refuses a folder without naming what makes it full.** *"is not empty: nstack init creates a new project. Action: pick a folder that is missing or empty"* — the reader then hunts, and the culprits are hidden files their editor and their agent put there (`.vscode`, `.idea`, `.mcp.json`, `.claude`). Refusing is right; not naming the place is P6. Found by the pilot on 2026-09-20, at the framework's very first gesture | **Done (2026-09-22)**: the refusal names the first entries it found and offers the folder inside, as a command — which is the decision the pilot's decider had to take by hand |
| D57 | **The kernel and PDR-0008 contradict each other on credentials.** The kernel says *"never with a human's credentials"*, without nuance; PDR-0008 says an agent may apply the forge's settings with a credential that **cannot commit**. Given exactly such a token — Administration write, `Contents: No access` — a fresh session read the kernel and refused to use it, then asked the decider. It was right to report rather than resolve, and the two texts still disagree. Found by the pilot on 2026-09-20 | **M11k**, with PDR-0008: either the kernel carries the exception or the settings command does |
| D58 | **A project receives its agent's machine state.** `.claude/` and `.mcp.json` appear in the project folder; the skeleton's `.gitignore` covers `.claude/skills/` and `.claude/settings.local.json`, so it already knows this agent, but neither of these. A `git add -A` commits them. The pilot hit it and wrote the ignore rule itself (its pull request #4). Found on 2026-09-21 | **Done (2026-09-22)**: the skeleton adopts the rule the pilot wrote for itself — what the tool writes as it runs is ignored, `.claude/settings.json` and `.claude/hooks/` are not, because a team shares those on purpose. The oracle checks both halves on a generated project |
| D59 | **The product's README is the framework's manual.** The skeleton writes 250 lines of installation manual under the project's own sentence, and says in them *"Replace it with your project's own README"* — which nothing checks and nobody does. The shop window of the product is the instructions for its tooling. Rails generates a stub, Django generates none; neither puts its own manual on the front page of what you build. Found by the maintainer reading the pilot, 2026-09-22 | A README stub, the foundation manual moved under `docs/os/`, and L5 checking it left. Planned with **PDR-0010** |
| D60 | **The framework declares issues a source of truth and never reads one.** `06-decisions.md` §9 says *"Work to be done → Issues / the project board"*, the skeleton ships five issue forms and a pull request template saying `Closes #` — and the engine reads only `docs/project/cycles/`, while K3 requires `Deliverable: D<n>`. Measured on the pilot: **23 pull requests, 0 issues**, which is the logical consequence of what it was given. What has no home today: work deferred by a failed scenario (its pull request #12), and what a round of work learned (its #19) | Decided by **PDR-0010** (accepted 2026-09-22): a reviewed file carries what binds the project, a typed issue carries what tracks it and what arrives unplanned, and one named issue per project carries what must be remembered. Implemented in **M11l** |
| D61 | **Neither the merge method nor branch cleanup is in the checklist.** The pilot's repository allows merge commits, while the whole method assumes squash — one workstream, one commit — and keeps every merged branch: **21 branches for 20 merged pull requests**. Both settings are readable through the API **without the Administration permission**, so a rule here can be verified rather than merely written. Found on 2026-09-22 | **Done (2026-09-24)**: **G15** squash as the only merge method, **G16** head branch deleted on merge, and **G14** auto-merge — the three read from the repository's own object, which needs **neither the Administration permission nor a paid plan**, so they hold where rulesets cannot (PDR-0006). Measured on the three repositories the day the rule landed: `engineering-os` G14 FAIL / G15 OK / G16 OK, `tool-library` and `flet` FAIL on all three — including ours, which is why every approval in that session still needed a human to come back and merge |
| D62 | **No release note is published anywhere.** 0 GitHub releases, no `CHANGELOG`, nothing on the PyPI page: the notes exist only in the body of each version's pull request, in the framework's own repository, where a project's team has no reason to look. PDR-0001 promises an update *"offered as a pull request to review"* — reviewed against what? Found by the pilot on 2026-09-22, which refused to take an unknown change mid-cycle and went to diff the packages instead | **Done (2026-09-22)**: `CHANGELOG.md` is the source, the release workflow refuses to publish a version with no section and then posts that section as the tag's release, and `nstack update` names where to read it |
| D63 | **The published package cannot show what an update brings.** The wheel and the source archive hold `src/napkinstack/` and not `skeleton/`, which Copier reads from the tag (ADR-0001). So reconstructing a version from the artefacts — the only method left by D62 — is **structurally blind to what `nstack update` merges**. Measured: the pilot diffed both packages of v0.6.1, saw one file, and concluded *"nothing that nstack update would merge moves"*. v0.6.1 changes `skeleton/.gitignore`, 16 lines, which will conflict with the rule it wrote itself. Its reasoning was sound; the artefacts betrayed it | **Done (2026-09-22)**: every entry separates **what changes in the engine** from **what changes in your project**, and says why the second cannot be read from the package. Not by shipping the skeleton in the wheel: it would duplicate what the tag already holds and diverge from it |
| D64 | **The suite's update test failed once on CI, and `main` went red.** `nstack update` stopped on `[Errno 39] Directory not empty: 'pack'` while the throwaway template was being read, so the conflict the test expects never came. One failure in sixty runs; **not reproduced** in three local passes, including one with a writable Copier cache — and the suite creates no Copier cache for a local template, so that hypothesis is dead. The cause is **not established**. Found on 2026-09-22, after merging #96 | git's automatic maintenance is disabled for the suite, through the environment: a repository thrown away within seconds has no business being garbage-collected while a temporary clone of it is removed. Justified on its own terms whatever the failure was; if it recurs, we know it was not that |
| D65 | **Every `OSError` was reported as "Template unreachable".** `[Errno 39] Directory not empty` is the filesystem answering, not a template out of reach, and the action offered — check `--source`, `--ref` and the network — could not help. It cost ten minutes of diagnosis on a red `main`, to the person who wrote the branch. Third instance of the same family, after D54 and D55. Found on 2026-09-22 | **Done**: an `OSError` carrying an errno names the filesystem and says to read what it names; the ones Copier raises for an unreadable template carry none, and keep their message |
| D66 | **An update named only the target version's release page.** One update can cross several versions, and the change that conflicts with a project's own files may come from any of them: the pilot, on v0.6.0, would have been sent to v0.6.2's page to explain a v0.6.1 change — the shape of D63, one layer up. Found planning M11, 2026-09-23 | **Done (2026-09-23)**: v0.6.2 (#99) — the notes point at the `CHANGELOG` at the tag and name every section above the version the project came from. **First used for real on 2026-09-24**, by the pilot's own update (its #31): the command printed the address and *"every section above v0.6.1"* |
| D67 | **Proportionality promised three times, executed by one threshold.** `07-governance.md` §6 (a diagram), `05-workflow.md` §7 (a seventeen-row Definition of Done) and `08-quality.md` §7 (a thirteen-row reliability table) described what each criticality requires in three shapes, and only the first knew `prototype`; the engine read one threshold, written in four places. Found planning M11 | **Done (2026-09-24)**: **M11a** (#102) — one matrix, `prototype` included, each row *machine* or *owed*, a test refusing a second description; **M11b** (#103) — `assurance.py` the single source that T1, M8 and `new-module` read |
| D68 | **Nothing says when to take an update.** A project that updates mid-cycle changes the judge while a cycle is being judged. The pilot answered it itself and kept the order — close the cycle (its #27), update at the boundary (#29), then accept the next (#30) — in its own words: *"upgrading mid-cycle changes the judge while a cycle is being judged."* Found by the pilot, 2026-09-23 | **Done (2026-09-24)**: **M11m** — the pilot's sentence, adopted as written where the skeleton's README describes an update, with the order it kept: close the cycle, take the update, accept the next |
| D69 | **The checklist imposes a level-3 control from the first commit.** A non-author human approval before merging (`OSPS-QA-07.01`) is level 3 of 3 in the OpenSSF OSPS Baseline; G2 and G3 require it of every project from day zero. We ship the top tier as the floor. Found by the market survey behind M11, 2026-09-23 | **M11g's probe, 2026-09-24: the mapping is not clean** — [record](audits/2026-09-24-osps-baseline-probe.md). Identifiers stable and levels readable, but six of our sixteen rules have no control. **Returned to M13**; the facts stand and D69 stays open |
| D70 | **The agent chose the criticality on a command line with no help.** `criticality` was a positional argument with no criteria at hand and no consequence printed before the choice; the pilot's read-only `screening` module, which holds no key and places no order, was declared `high`. Measured on the pilot while planning M11, 2026-09-23 | **Done (2026-09-24)**: **M11c** (#106) — help phrased as consequences, the price of the value and of its neighbour printed, the creation ADR asked for one sentence of reason, and one kernel line routing the choice to the decider |
| D71 | **T4's ratchet: one fix invalidated every scenario.** Any commit after verification required every row re-run at the head, so each fix cost a full round; the pilot ran six on one read-only `high` module, and three of its eleven defects were introduced in the late rounds. Measured on the pilot while planning M11, 2026-09-23 | **Done (2026-09-24)**: **M11d** (#104) — an automated scenario is re-run, an explored one is **confirmed** at the head with what moved since, and `critical` re-runs everything |
| D72 | **A criterion nobody can meet is paid in counterfeit.** C4 required acceptance criteria on a deliverable whose outcome waited on people outside the project, and the pilot was asked five times to simulate the evidence. Measured on the pilot while planning M11, 2026-09-23 | **Done (2026-09-24)**: **M11e** (#105) — C4's refusal names the **spike**, the door `05-workflow.md` §3 already had, and *"a dependency with a date"*. The plan was wrong: moving such work to an issue would have contradicted §3 |
| D73 | **Our own migration note over-predicted a conflict.** v0.6.1's note announced that a project holding its own agent ignore rules *"will conflict on `.gitignore`"*, and v0.6.2's repeats it for a project coming from v0.6.0. The pilot's update was a two-line diff with no conflict (its #29), because D58 had adopted its own text verbatim — the one project we knew about is the one where the prediction was false, and it then spent a check proving nothing had been silently dropped (its #31). Found 2026-09-24 | **M11i**: v0.7.0's migration paragraph is written from an update actually run against a copy of each real project before the tag, not predicted from the diff. The published notes are not rewritten |
| D74 | **`nstack update` branches from `HEAD` without saying so.** The pilot was on `cycle/03-framing`; the update branch carried a foreign commit and had to be replanted onto `main` by hand (its #29), and its handover now warns *"check out `main` first, or expect to replant the branch."* Found by the pilot, 2026-09-23 | **Done (2026-09-24)**: **M11m** — `nstack update` refuses when another branch than the default one is checked out, before anything is written, and names both branches and `git switch <default>`. Refused rather than switched for the user: changing a checkout silently is the same shape as the defect |
| D75 | **Seventy-five green tests were green on a path nobody takes.** All 20 `main(...)` calls in the pilot's suite passed `client=`, so the one line an administrator's own invocation runs was exercised by no test, and its first blocker survived six rounds because of it (its #26). **Not a check**: P1 forbids stack-specific logic. Found 2026-09-24 | **Done (2026-09-24)**: **M11m** — `playbooks/verification.md`, at the step where the verifier reads the diff: *is the entry point a real user calls exercised the way they call it?* A playbook line, not a check (P1) |
| D76 | **An owed row gets forgotten.** Observability deferred in two cycles running (its #26: *"carried from cycle 01"*). The matrix says honestly that no check reads that row — and nobody read it. Found 2026-09-24 | Measured before judged: not scheduled. PDR-0011's criterion is where a pattern would show; a check is not the answer to a row declared *owed* |
| D77 | **`nstack doctor`'s exit code has one value for two states.** With no token it reports *"0 gap(s), 13 not verified"* and exits 1, as it does on a real gap. Refusing to call unreadable settings compliant is right (PDR-0006); a script, a CI step or an agent reading the code cannot tell *"not read"* from *"not in place"*, and the pilot learned to read the count instead (its #30). Found by the pilot, 2026-09-23 | **M11k**, beside the settings command that shares `doctor`'s checklist |
| D78 | **The checklist never protects the default branch from deletion or force-push.** `OSPS-AC-03.02`, level 1 of the OSPS Baseline, asks that deleting the primary branch be treated as sensitive; this repository's own ruleset forbids both deletion and force-push on `main`, and step 0 above says so — but no rule G1 to G16 asks a generated project for either, and `doctor` reads neither. Found by M11g's probe, 2026-09-24 | **M11k**, with the settings command that shares the checklist |
| D79 | **The framework's own identifiers reach a project unexplained.** Engine output and skeleton files cite NapkinStack's decision records — *"(PDR-0005)"*, *"(PDR-0001 R3)"*, `ADR-0001` in `.pre-commit-config.yaml`, `PDR-0006` in `commits-on-main.yml` — which collide with the project's own `docs/pdr/` and `docs/adr/` numbering, and `module-checks.yml` cites *"(D46)"* where a project's D-numbers are its cycle deliverables. Rule codes the next steps name (T1–T5, K1, K4, M7) are defined in no document the project holds; `doctor` reports *"[L4] PRODUCT.md absent"*, a file the project never heard of; and two measurements from this framework's pilot sit in the skeleton's handbook. Found by the coherence pass of 2026-09-24, on a generated project | **M11l**, with PDR-0010: the same boundary between what the framework says and what the project owns |
| v0.6.0 | **What is announced refuses, and the documents say what is true**: D51 to D54, and the documentary lot of the internal audit — [plan](plans/2026-09-20-v0.6.0-what-is-announced-refuses.md) | **Done (2026-09-20)**: five workstreams merged (#83, #84, #85, #86, #87), **v0.6.0 published**, tag on `8ae544c`, verified on published artefacts — including an update of `tool-library` from v0.5.0, 9 files and no conflict, still compliant. Found doing it: D55. Next: the pilot, and M11 beside it |
| PDR-0007 | [Frame a project when framing buys something](../pdr/0007-frame-a-project-when-framing-buys-something.md) | **Accepted (2026-09-22)**: a project with no accepted charter is **unframed**, the cycle rules K1 to K3 do not apply there, everything else does, and every judging run names the state. Measured: today a first delivery pull request hits `K1` with no way out but a justified label on each one. Planned for **M11**; criterion 2027-03-31. **Not carried by M11's first plan**, although all three records said M11: found by M11h, and scheduled on 2026-09-24 as **M11j**, before v0.7.0. |
| PDR-0008 | [Configure the repository in one gesture, without handing over the keys](../pdr/0008-configure-the-repository-in-one-gesture.md) | **Accepted (2026-09-22)**, and it absorbs D61: the thirteen forge settings become one plan-aware gesture that only ever strengthens; printed by default, applied by an agent only with a credential that **cannot commit**, which the command verifies before acting (ADR-0004, clarified). Planned for **M11**; criterion 2027-03-31. **Not carried by M11's first plan**, although all three records said M11: found by M11h, and scheduled on 2026-09-24 as **M11k**, before v0.7.0. Its D61 half is done (M11f: G14, G15, G16); the command is not |
| Pilot | **`NapkinStack/flet`, the first real project**, run by a session that knew nothing of this repository, with the maintainer as decider and this repository's agent as observer — public on the Free plan, where the whole checklist is available, rather than private on Team where G5 is paid and G6 does not exist | **Running since 2026-09-20**: discovery decided *go* after three rounds and two independent challenges, charter accepted; cycle 01 closed `stopped/reframed`, **cycle 02 closed** with two deliverables accepted and one deferred (its #27), **cycle 03 framed and approved** (#28, #30), which starts its first `critical` module; the framework taken to v0.6.1 by itself at the boundary (#29), v0.6.2 proposed (#31). It has produced D56 to D63, D68, D70 to D77, replicated D26 and D55, and measured D49 — the first figures on what an independent verifier finds (PDR-0003) |
| PDR-0010 | [The framework imposes the form, the project writes the content](../pdr/0010-the-framework-imposes-the-form.md) | **Accepted (2026-09-22)**, proposed by the findings above. `00-overview.md` §5 already says the framework never names a stack, a tool or a business domain; D59, D60 and D61 are three places where it breaks its own rule. One decision covers what a README holds, what an issue carries against what a reviewed file carries, and what the framework may require of a product's documentation without writing it. **Not carried by M11's first plan**, although all three records said M11: found by M11h, and scheduled on 2026-09-24 as **M11l**, before v0.7.0. |
| PDR-0011 | [Ceremony follows consequence](../pdr/0011-ceremony-follows-consequence.md) | **Accepted (2026-09-24)**, proposed in M11h, recording what M11's plan decided and what its first six workstreams built: two axes, two owners — assurance per module, exposure per project — one matrix saying when, one round, the price shown before it is paid. Criterion 2027-03-31 |
...
| M11 | **Ceremony follows consequence** (PDR-0011): the framework charges what a failure costs, per module and per project, and states each price before it is paid; with PDR-0004's routing, and PDR-0007, PDR-0008 and PDR-0010 — through to v0.7.0 — [plan](plans/2026-09-23-m11-ceremony-follows-consequence.md) | **In progress.** Planned 2026-09-23 (#98). Merged: **M11f** (#101), **M11a** (#102), **M11b** (#103), **M11d** (#104), **M11e** (#105), **M11c** (#106), **M11h** (#107), **M11g** (#108) closed by its probe — not clean, returned to M13; **M11m** (D68, D74, D75). To do: **M11j** PDR-0007, **M11k** PDR-0008, D57, D77 and D78, **M11l** PDR-0010, then **M11i**, v0.7.0 |
| Review 2026-09-24 | **A coherence pass after M11a to M11h**: four independent reads — the skeleton against the engine, this repository's records, the engine against itself, and a project generated and driven end to end | **Done (2026-09-24)**, in one pull request: the matrix's `critical` paragraph matched to its table, a test binding the table's machine rows to `assurance.MATRIX`, an unknown criticality named as read as critical in CI and in M8, the sheet's trigger stated alike everywhere, auto-merge said to be armed per pull request, a partial read of G14–G16 reported as not verified, and the records' stale states. D79 found and scheduled |
...
| M13 | **The merge record** (PDR-0009), and the checklist mapped to the OpenSSF OSPS Baseline's control identifiers | After the pilot's first cycle, which supplies the twenty pull requests its criterion measures — **satisfiable since 2026-09-23**: the pilot's cycle 02 closed with its measurements (its #27). The OSPS mapping **is back here**: M11g's probe found it not clean on 2026-09-24, and its table is M13's starting point — [record](audits/2026-09-24-osps-baseline-probe.md) |
