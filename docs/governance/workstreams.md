...
| D53 | **A contract change never runs the other side's checks.** `03-contracts.md` §5 promises *"contract tests run in the CI of both modules"* and that a change breaking a declared consumer is caught before merge. `module-checks.yml` runs the verbs for modules with a **changed file**, so a change under `contracts/` runs the contracts module's tests and nobody else's; V1 compares the documents, it runs neither side's suite. Found by the same audit | v0.6.0: **d** — `nstack modules --with-contract-sides` adds the producer and the declared consumers of a touched version, and the workflow asks for them |
