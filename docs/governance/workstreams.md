...
| D74 | **`nstack update` branches from `HEAD` without saying so.** The pilot was on `cycle/03-framing`; the update branch carried a foreign commit and had to be replanted onto `main` by hand (its #29), and its handover now warns *"check out `main` first, or expect to replant the branch."* Found by the pilot, 2026-09-23 | **M11m** |
| D75 | **Seventy-five green tests were green on a path nobody takes.** All 20 `main(...)` calls in the pilot's suite passed `client=`, so the one line an administrator's own invocation runs was exercised by no test, and its first blocker survived six rounds because of it (its #26). **Not a check**: P1 forbids stack-specific logic. Found 2026-09-24 | **M11m**: one question in `playbooks/verification.md` — is the entry point a real user calls exercised the way they call it? |
...
| M11 | **Ceremony follows consequence** (PDR-0011): the framework charges what a failure costs, per module and per project, and states each price before it is paid; with PDR-0004's routing, and PDR-0007, PDR-0008 and PDR-0010 — through to v0.7.0 — [plan](plans/2026-09-23-m11-ceremony-follows-consequence.md) | **In progress.** Planned 2026-09-23 (#98). Merged: **M11f** (#101), **M11a** (#102), **M11b** (#103), **M11d** (#104), **M11e** (#105), **M11c** (#106), **M11h** (#107), **M11g** (#108) closed by its probe — not clean, returned to M13. To do: **M11j** PDR-0007, **M11k** PDR-0008, D57, D77 and D78, **M11l** PDR-0010, **M11m** D68, D74, D75, then **M11i**, v0.7.0 |
