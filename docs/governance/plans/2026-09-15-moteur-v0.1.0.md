...
- [ ] **Étape 1 : tests qui échouent.** Dans `platform/tests/run.sh`, avant
  `echo "Tests plateforme : OK"` :
...
- [ ] **Étape 2 : voir échouer.** Attendu : `ÉCHEC : checklist absente de la sortie d'init.`
- [ ] **Étape 3 : implémenter.** Créer `src/napkinstack/doctor.py` :
...
CHECKLIST = [  # (règle, réglage, action)
    ...
    ("G11", "Labels " + " et ".join(f"`{label}`" for label in LABELS),
     "Settings → Issues → Labels : créer " + " et ".join(LABELS)),
]
...
- [ ] Au moins 1 relecture approuvée
- [ ] Revue des CODEOWNERS obligatoire
- [ ] Checks obligatoires : `Fitness functions`, `Périmètre et budget de revue`, `Hooks et secrets`
...
- [ ] Actions autorisées : celles de GitHub, plus `astral-sh/setup-uv`
...
- [ ] Labels `cross-module` et `hors-budget` : ils rendent les exceptions visibles **et
...
- [ ] **Étape 5 : commit.** `git add -A && git commit -m "M3 — nstack doctor et checklist d'init"`

### Tâche 2 — Documentation et suivi

- [ ] **Étape 1.** `platform/README.md` : tableau « Contenu », ligne
...
- [ ] **Étape 2.** `docs/governance/chantiers.md` : nœud `RP` →
  ...
  (M2b) et `nstack doctor` (M3) ; le README n'a plus de décompression ni de
  `gh repo create`. »
- [ ] **Étape 3 : vérifier.** Hooks sur tous les fichiers ; tests verts.
- [ ] **Étape 4 : commit.** `git add -A && git commit -m "M3 — documentation et suivi"`

### Tâche 3 — Vérification, PR et merge

- [ ] **Étape 1 : vérification complète.** Fitness, tests en local et en GitHub simulé,
  hooks, scan d'historique, `pr-scope`.
- [ ] **Étape 2 : vraie API, en lecture seule.** Projet d'essai dont `github_repo` est
...
- [ ] **Étape 3 : mutations.** Inverser une vérification (G8) et le traitement de l'absence de
  jeton : les tests correspondants échouent.
- [ ] **Étape 4 : rejeu sur clone vierge** sous un chemin long, en GitHub simulé.
- [ ] **Étape 5 : fuites.** Identité unique ; diff sans adresse, jeton ni chemin local.
- [ ] **Étape 6 : PR** « M3 — nstack doctor », résumé `FAIT / VÉRIFIÉ / SUPPOSÉ / NON VÉRIFIÉ /
  RISQUES`, label `hors-budget` si le budget est dépassé.
- [ ] **Étape 7 : merge.** Checks verts, `CLEAN`, aucun commentaire, rejeu sur clone GitHub au
  SHA de tête ; `gh-napkinstack pr merge --squash --match-head-commit <sha>`.
