...
> M2a, M2b, M3 et M3.1 (faits), puis M3.2.
...
flowchart LR
    ...
    M31 --> M32["M3.2<br/>alignement docs"]:::encours
    M32 --> M4["M4<br/>module sans stack"]:::afaire
...
## M3.2 — Alignement de la documentation
...
- [ ] **Étape 1 : test qui échoue.** Dans `platform/tests/run.sh`, après le bloc
...
- [ ] **Étape 2 : voir échouer.** Attendu : la liste des ~40 renvois morts.
- [ ] **Étape 3 : corriger.** Dans les fichiers du squelette hors manuel : `docs/0X-` →
...
- [ ] **Étape 4 : voir passer, commit.** `git commit -m "M3.2 — renvois au manuel corrigés et gardés (D6)"`

### Tâche 2 — Positionnement, vocabulaire, schémas

- [ ] **Étape 1.** `PRODUCT.md` : en-tête (« projet créé par `nstack init` ») ; §1 positionnement,
  ...
  réel.
- [ ] **Étape 2.** README racine : première phrase « Framework de travail… », bandeau d'état
  (`init`, `update`, `doctor`), commande `doctor` dans « Développer NapkinStack » ;
  `pyproject.toml` : même description.
- [ ] **Étape 3.** README du squelette : présentation « créé par `nstack init` », schéma du cycle
  de vie légendé, « ni framework applicatif ».
- [ ] **Étape 4.** Manuel : `00-vue-ensemble.md` §5 « framework applicatif » et glossaire
...
- [ ] **Étape 5.** `platform/README.md` : en-tête en double supprimé ; `chantiers.md` : nœud
  `RP`, D6, D19, D22.
- [ ] **Étape 6 : vérifier et commiter.** Hooks, tests ; relecture croisée des termes
...
- [ ] Fitness, tests en local et en GitHub simulé, hooks, scan d'historique ; rejeu sur clone
