...
```mermaid
flowchart LR
    M1["M1<br/>moteur empaqueté"]:::fait --> M2a["M2a<br/>squelette Copier"]:::encours
    M2a --> M2b["M2b<br/>init, update"]:::afaire
...
- [ ] **Étape 2 : tests qui échouent.** Dans `platform/tests/run.sh` :
...
- [ ] **Étape 3 : voir échouer.** `uv run bash platform/tests/run.sh` — attendu :
  `ÉCHEC : la racine donnée n'est pas analysée.` (le moteur cherche encore
  `platform/skills.yaml`).
- [ ] **Étape 4 : implémenter.** Dans `src/napkinstack/skills.py` : docstring,
...
- [ ] **Étape 5 : voir passer.** `uv run bash platform/tests/run.sh` vert ;
  `uv run nstack fitness --root .` affiche `Skills : non applicable`.
- [ ] **Étape 6 : commit.** `git add -A && git commit -m "M2a — skills configurées dans .nstack/, playbooks dans le squelette"`

### Tâche 2 — Gabarit Copier : réponses rendues, contexte NapkinStack absent

- [ ] **Étape 1 : dépendance.** `pyproject.toml` :
  `dependencies = ["PyYAML>=6.0", "copier==9.18.2"]`, puis `uv lock`.
- [ ] **Étape 2 : tests qui échouent.** À la fin de `platform/tests/run.sh`, avant
...
- [ ] **Étape 3 : voir échouer.** Attendu : sans `copier.yml`, Copier copie tout le dépôt,
  puis `ÉCHEC : .github/CODEOWNERS ne contient pas « @acme/plateforme ».`
- [ ] **Étape 4 : gabarit.** Créer `copier.yml` :
...
- [ ] **Étape 8 : racine.** Créer `README.md` :
...
- [ ] **Étape 9 : voir passer.** `uv run bash platform/tests/run.sh` vert jusqu'au dernier
  bloc ; `uv run nstack fitness --root .` vert (un seul manifest : `platform`).
- [ ] **Étape 10 : inventaire du squelette.** Cette commande ne doit rien afficher :
...
- [ ] **Étape 11 : commit.** `git add -A && git commit -m "M2a — gabarit Copier du squelette de projet"`

### Tâche 3 — CI et hooks du projet généré

- [ ] **Étape 1 : tests qui échouent.** À la suite du bloc de la tâche 2 :
...
- [ ] **Étape 2 : voir échouer.** Attendu : `ÉCHEC : skeleton/.pre-commit-config.yaml diverge
  de .pre-commit-config.yaml.`
- [ ] **Étape 3 : hooks.** Dans `.pre-commit-config.yaml`, la phrase « l'aligner sur celle du
...
- [ ] **Étape 4 : outils.** `pyproject.toml` : `pre-commit==4.6.2` passe dans `dependencies`,
  la table `[dependency-groups]` disparaît ; `uv lock`.
- [ ] **Étape 5 : CI du projet.** Créer `skeleton/.github/workflows/governance.yml` :
...
- [ ] **Étape 6 : voir passer.** `uv sync --locked && uv run bash platform/tests/run.sh` vert,
  ...
  `skeleton/`).
- [ ] **Étape 7 : commit.** `git add -A && git commit -m "M2a — CI et hooks du projet généré"`
...
git grep -n -E "(^|[^/])(docs/os/|playbooks/)|platform/(skills\.yaml|tooling-profile\.md)" -- . \
  ...
  ':!.github/ISSUE_TEMPLATE/0*' ':!.github/pull_request_template.md'
```

  Exclusions : squelette et messages du moteur, écrits pour un projet ; décisions et
  historique ; copies de référence des formulaires.
- [ ] **Étape 2 : Dependabot de la racine.** Dans `.github/dependabot.yml`, les blocs
...
- [ ] **Étape 3 : corriger chaque occurrence.**
  ...
  - `.github/workflows/governance.yml` : en-tête, `docs/os/07-gouvernance.md` →
    `skeleton/docs/os/07-gouvernance.md`.
  - `PRODUCT.md` §1 : première ligne du tableau → `` `skeleton/` : `AGENTS.md`, `playbooks/`,
    `docs/os/`, `contracts/`, CI et hooks `` ; deuxième → `` `src/napkinstack/`, `copier.yml`,
    `platform/`, `.github/` `` ; P2 → `` les règles du squelette (`AGENTS.md`, `playbooks/`,
    `docs/os/` sous `skeleton/`) `` ; §5 → `` le kernel `skeleton/AGENTS.md` `` ; §6 →
    `` `skeleton/docs/os/07-gouvernance.md` §3 ``.
...
- [ ] **Étape 4 : inventaire vert.** La commande de l'étape 1 n'affiche plus rien ;
  `uv run pre-commit run --all-files` vert (check-dependabot valide `group-by`).
- [ ] **Étape 5 : commit.** `git add -A && git commit -m "M2a — Dependabot sur deux répertoires, références et suivi"`

### Tâche 5 — Vérification, PR et merge

- [ ] **Étape 1 : vérification complète.** `uv run nstack fitness --root .` ;
...
- [ ] **Étape 2 : rejeu sur clone vierge** de la branche, dans le scratchpad : `git clone`,
  `uv sync --locked`, tests en GitHub simulé.
- [ ] **Étape 3 : fuites.** Identité unique des commits
  (`git log --format='%an <%ae>' origin/main..HEAD | sort -u`) ; relecture du diff à la
  recherche d'adresses, de jetons et de chemins locaux.
- [ ] **Étape 4 : checks obligatoires.** Lire le ruleset par `gh-napkinstack api` : aucun
  check du workflow « Modules », retiré de la racine, n'y est exigé.
- [ ] **Étape 5 : PR.** Push par l'alias SSH ; PR « M2a — Squelette Copier » par
...
- [ ] **Étape 6 : merge.** Checks requis verts, PR `CLEAN`, aucun commentaire ; rejeu sur un
...
- **Couverture de la spécification :** R2/R3 (M2a : `_commit` lu par la CI du projet ;
  M5 : vérifié sur PyPI), R4 (M2b : critère 7), R5 (M1 : uv ; M4 : module), R6 (M2a : test
  d'absence), critères 1 et 4 à 7 (M2b), 2 (M3), 3 (M4), permissions en lecture seule (M3),
...
- **Marqueurs de remplissage :** aucun dans M1 et M2a ; les chantiers M2b à M6 sont décrits
  par livrable, leur plan détaillé suivra.
- **Cohérence des interfaces :** `run(root: Path) -> int` pour les trois contrôles,
  ...
  dans `copier.yml`, les `.jinja` et les tests.
