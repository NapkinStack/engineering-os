...
> M2a, M2b, M3, M3.1, M3.2, M4 et M6 (faits), puis M6.1.
...
flowchart LR
    ...
    M6 --> M61["M6.1<br/>fiabilité des contrôles"]:::encours
...
## M6.1 — Fiabilité des contrôles hérités
...
- [ ] **Étape 1 : outil.** `pyproject.toml` : `[dependency-groups]` avec `dev = ["pytest==9.1.1"]` ;
...
- [ ] **Étape 2 : tests qui échouent.** Créer `platform/tests/test_controles.py` :
...
- [ ] **Étape 3 : voir échouer.** `uv sync && uv run pytest -q platform/tests` — attendu : M1
  absent de la sortie, et les cas D8 en erreur (`AttributeError`).
- [ ] **Étape 4 : implémenter.** Dans `src/napkinstack/fitness/manifests.py` :
...
- [ ] **Étape 5 : voir passer ; mutations.** Tests verts ; pour M3 à M9, désactiver la règle
  (condition à `False`) fait échouer son cas ; restaurer.
- [ ] **Étape 6 : commit.** `git commit -m "M6.1 — manifests : M1 implémenté, types vérifiés, règles M prouvées"`

### Tâche 2 — Frontières : B1 à B5, D8

- [ ] **Étape 1 : tests.** Ajouter à `platform/tests/test_controles.py` :
...
- [ ] **Étape 2 : voir échouer.** Attendu : le cas du manifest illisible en erreur
  (`AttributeError`) ; B1 à B5 verts, car déjà implémentés.
- [ ] **Étape 3 : implémenter.** Dans `load_modules` de `boundaries.py` :
...
- [ ] **Étape 4 : mutations, commit.** Désactiver chacune des règles B1 à B5 fait échouer son
...
- [ ] **Étape 2 : voir échouer.** Attendu : les cas D8 de skills en erreur ; S2, P1, P2 verts.
- [ ] **Étape 3 : implémenter.** Dans `skills.run`, la lecture de la correspondance devient :
...
- [ ] **Étape 4 : mutations, commit.** Désactiver S2 et P1, forcer le seuil P2 : les cas
...
- [ ] `docs/governance/chantiers.md` : D4, D5, D8 → « M6.1 » traités ; nœud `RP` → « M6 fait,
