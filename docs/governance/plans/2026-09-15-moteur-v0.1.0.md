...
flowchart LR
    ...
    M3 --> M31["M3.1<br/>dépôts privés"]:::encours
    M31 --> M4["M4<br/>module sans stack"]:::afaire
...
- [ ] **Étape 1 : tests qui échouent.** Dans le générateur de `routes.json` de
...
- [ ] **Étape 2 : voir échouer.** Attendu : `ÉCHEC : dépôt privé sans barrière accepté.` ou
  `… mal rapporté.` (G6 évalué au lieu d'être non applicable).
- [ ] **Étape 3 : implémenter.** Dans `src/napkinstack/doctor.py` :
...
- [ ] **Étape 4 : voir passer.** Tests verts, puis en GitHub simulé. *Écart constaté à
  l'exécution : le test d'identité de la checklist exige la nouvelle ligne G6 du README dès
  cette tâche ; elle y est faite.*
- [ ] **Étape 5 : commit.** `git add -A && git commit -m "M3.1 — doctor suit la visibilité du dépôt"`

### Tâche 2 — Prérequis, avenant et suivi

- [ ] **Étape 1 : README du squelette.** Avant « ## Installation » :
...
- [ ] **Étape 2 : `skeleton/SECURITY.md`.** Après « Le rapport n'est visible que des
...
- [ ] **Étape 3 : avenant.** À la fin de PDR-0001, avant « Condition de retrait », section
...
- [ ] **Étape 4 : suivi.** `docs/pdr/README.md` : statut « Accepté, précisé (dépôts privés) » ;
...
- [ ] **Étape 5 : vérifier et commiter.** Hooks, tests verts ;
  `git add -A && git commit -m "M3.1 — prérequis, avenant et suivi"`

### Tâche 3 — Vérification, PR et merge

- [ ] **Étape 1.** Fitness, tests en local et en GitHub simulé, hooks, scan d'historique.
- [ ] **Étape 2 : vraie API.** `doctor` sur `NapkinStack/engineering-os` (public) : mêmes
  résultats qu'à M3, G6 conforme.
- [ ] **Étape 3 : mutation.** Désactiver le statut non applicable : le test du dépôt privé
  sous Team échoue.
- [ ] **Étape 4.** Rejeu sur clone vierge ; fuites ; PR « M3.1 — Dépôts privés » ; merge après
