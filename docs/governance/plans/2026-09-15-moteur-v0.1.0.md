...
- [ ] **Étape 1 : l'oracle reste dans le module.** `platform/tests/run.sh` ne bouge pas : M9
...
- [ ] **Étape 5 : vérifier.** `uv run bash platform/tests/run.sh` vert ; `uv run pre-commit run --all-files`
...
- [ ] **Étape 4 : vérification complète.** Fitness, `uv run bash platform/tests/run.sh` (local et
