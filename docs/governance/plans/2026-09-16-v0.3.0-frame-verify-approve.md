...
- [x] **Maintainer**: Organisation settings → Developer settings → GitHub Apps → New GitHub
...
- [x] **Maintainer**: generate a private key; install the App on `engineering-os` only.
- [x] **Maintainer**, in their own terminal:
```bash
install -d -m 700 ~/.config/napkinstack-agent
mv "$KEY" ~/.config/napkinstack-agent/private-key.pem   # $KEY: the file GitHub downloaded
...
- [x] **Maintainer**: Settings → Rules → Rulesets → `main` → "Require a pull request before
      merging": 1 required approval, "Require review from Code Owners". The bypass list
      stays empty.
- [x] **Agent**: verify through the API (`gh-napkinstack`, for the last time):
      `required_approving_review_count` 1, `require_code_owner_review` true,
      `bypass_actors` empty.

**Deviation (2026-09-17):** the App was first created without Contents — seen by reading
the installation's permissions with the App's own JWT, then added by the maintainer and
accepted on the installation. The private key lands in the browser's download folder, whose
name follows the desktop's language.
...
- [x] Download `gh-token`'s latest release for linux-amd64 from
...
- [x] `~/.config/napkinstack-agent/app.env` (mode 600):
```bash
APP_ID=<the App ID>
INSTALLATION_ID=<the installation ID>
KEY=$HOME/.config/napkinstack-agent/private-key.pem
```
- [x] `~/.local/bin/gh-agent` — the GitHub CLI under the App's identity; a token minted per
...
```
- [x] `~/.local/bin/git-credential-napkinstack-agent` — git's credentials for github.com,
      the same token:
```bash
#!/usr/bin/env bash
...
```
- [x] Verify: `gh-agent api repos/NapkinStack/engineering-os --jq .full_name` answers;
      `git ls-remote origin` answers; `gh-agent auth status` is refused.

**Deviations:** `gh-token` v2.0.10 publishes neither a checksum file nor an attestation: the
binary was checked against the SHA-256 digest GitHub records for the release asset. Its
`--token-only` flag replaces the JSON parsing. Both scripts unset `GITHUB_TOKEN`,
`GH_ENTERPRISE_TOKEN`, `GITHUB_ENTERPRISE_TOKEN` and `GH_HOST`, as the previous wrapper did;
the credential helper also requires `https`. `git ls-remote` answers without credentials on
a public repository, so the helper's use was proven by `GIT_TRACE` on `git push --dry-run`.

### Task 3 — No human credential within the agent's reach

- [x] **Maintainer**: revoke the fine-grained token `napkinstack-engineering-os` on GitHub,
      then `secret-tool clear service napkinstack-gh`.
- [x] **Maintainer**: remove the SSH key of `napkinstack-admin` used by the
      `github-napkinstack` alias, on GitHub (Settings → SSH keys) and from `~/.ssh`.
- [x] **Maintainer**: in the Claude in Chrome extension, withdraw the permission on
...
- [x] **Maintainer**: in `~/.claude/settings.json`, the sandbox made strict:
```json
{
  "sandbox": {
    "enabled": true,
...
```
- [x] **Agent**, from its session, each expected to fail:
...
- [x] **Agent**: `gh-napkinstack` removed from `~/.local/bin`; its PreToolUse hook
      blocking a bare `gh` kept, and extended to refuse `gh-napkinstack`.

**Deferred by the maintainer (2026-09-17):** the agent keeps `napkinstack-admin`'s token,
for reading and administration, while its work under the App is validated; the
revocations and the strict sandbox come later, before the pilot's `nstack init`, where
ADR-0004's criterion is observed. Until then, this repository does not meet ADR-0004's
"agent's session" row: the approval is a barrier for the App, not yet for the agent.
Found while preparing this task — the sandbox above is not enough on Linux: the Read tool is
not sandboxed (`permissions.deny` on `Read(~/.ssh/**)`), sandboxed commands need the uv and
pre-commit caches (`sandbox.filesystem.allowWrite`), and without the optional seccomp filter
Unix sockets stay open — the desktop's SSH agent and keyring among them.

**Done on 2026-09-17, as M9's Task 0** ([the M9 plan](2026-09-17-team-mode-validation.md)),
observed from the agent's session after the restart:

| Credential | State | Observed |
|---|---|---|
| The token `napkinstack-engineering-os` | Revoked | `GET /user` with it: 401. Its dead keyring entry is out of the sandbox's reach either way |
| The alias's SSH key | Removed from GitHub; the key file kept by the maintainer, its `Host` block commented | The account lists no SSH key (public API); `~/.ssh` appears empty to the agent's commands, and the file tools are denied |
| The desktop's SSH agent and keyring | Unreachable | `ssh-add -l` fails; the session bus is blocked |
| Claude in Chrome | No page readable, github.com included | Reading github.com and a neutral site both refused; navigation alone still possible |
| `gh-napkinstack` | Deleted; the hook refuses it | — |

Still working: `gh-agent`, `git fetch`, the suite, the hooks and the history scan. **Two
constraints of the strict sandbox, for every later task:** network access is declared per
command (`github.com`, `api.github.com`, the package index); and git cannot write a
repository's configuration from inside it (`.git/config.lock` is masked) — remotes,
upstreams, identity and credential helper go through `git -c`, or are set by the maintainer.
...
- [x] `.github/CODEOWNERS`, before the foundation's rules:
```
# Default owner: every path the rules below do not name (ADR-0004).
*                              @NapkinStack/maintainers
```
- [x] `CONTRIBUTING.md`, the list of how to work gains:
```markdown
...
- [x] Commit under the App's identity, push over HTTPS, open the pull request with
      `gh-agent`.
- [x] **ADR-0004's criterion, observed on this repository**: with CI green and no approval,
...
- [x] **Maintainer**: reviews and approves in the browser. **Agent**: merges with
      `--match-head-commit <sha>`.

**Observed on pull request #41 (2026-09-17)**, CI green, no approval, as the App:
`gh-agent pr merge` stops client-side ("the base branch policy prohibits the merge"); the
REST merge endpoint answers 405 "Repository rule violations found — Waiting on code owner
review from NapkinStack/maintainers"; `pr review --approve` answers "Can not approve your own
pull request"; a no-op update of ruleset `main` answers 403 "Resource not accessible by
integration". **Deviation:** the agent's attempt was itself refused before reaching GitHub —
Claude Code's auto mode classified it as self-approval — so the maintainer ran the same
commands as the App from the session's shell prompt. A second barrier, on the agent's side;
the refusals recorded are GitHub's.

---

## Release v0.3.0 (PR 8)

- [x] Version pull request: `uv version --bump minor` (0.2.0 → 0.3.0); approved; merged.
      Minor, and breaking: `module.user_facing` is required (M10), and generated projects
      gain a fourth required check.
- [x] **Maintainer's explicit consent**, then the annotated tag `v0.3.0` on the merge
      commit, pushed by the agent under its App.
- [x] **Maintainer**: approves the `pypi` deployment.
- [x] Verification, as at v0.2.0: PyPI JSON; `pypi-attestations verify pypi --repository
...
- [x] `nstack update` from v0.2.0 to v0.3.0 on an adapted project: `CODEOWNERS` gains its
      default owner, merged; the module's manifest fails M10 until `user_facing` is
      declared; the new workflow and templates arrive.
- [x] Summary pull request: `PRODUCT.md` §3 — the tech lead's criterion becomes "create a
...
**Observed on 2026-09-17** (version pull request #42, tag `v0.3.0` on `3ffc60b`, run
35205062675):

| Verification | Result |
|---|---|
| The tag | Annotated, tagger `napkinstack-agent[bot]`, pushed with the App's token: `Release` triggered, `Build` green (tag and version identical), `Publish to PyPI` waiting on the maintainer, green once approved |
| PyPI | `napkinstack` 0.3.0, MIT licence, requires Python 3.12, a wheel and a source archive |
| Attestations | `pypi-attestations verify pypi --repository https://github.com/NapkinStack/engineering-os`: OK for both files; publisher GitHub, `release.yml`, environment `pypi`; the certificates name `release.yml@refs/tags/v0.3.0` and commit `3ffc60b`. The `pypi:<file>` form finds the wheel but not the source archive: its URL works |
| A fresh workstation (`UV_TOOL_DIR`, `UV_TOOL_BIN_DIR`, `UV_CACHE_DIR` isolated, `UV_NO_CONFIG`) | `uv tool install napkinstack==0.3.0 --with-executables-from pre-commit --refresh`: `nstack 0.3.0` and `pre-commit` |
| `nstack init` from GitHub, a team owner and a user owner | `_commit: v0.3.0`, "Project created, NapkinStack v0.3.0"; `CODEOWNERS` opens with `*` and the owner; the checklist lists `Test sheet and cycle`, the `out-of-cycle` label and the empty bypass list; no accented letter, no word of M8.0's search |
| The generated projects, their CI replayed | `nstack fitness`, the hooks, the history scan green; `nstack plan`: "the project is not framed yet"; `doctor` L1–L4 and L6 green, L5 until the README sentence is written, G1–G12 not verified without a token; `new-module` with a team and with a user; `pr-scope` within budget; `pr-check` explicit: K1 on an unframed project, T1 once the module is user-facing, K4 on an unjustified `out-of-cycle` label |

**`nstack update` from v0.2.0 to v0.3.0.** A project created by `nstack` 0.2.0 from its
published tag, adapted by the team (the README sentence, a section appended to
`AGENTS.md`, a `catalog` module owned by another team), then `nstack` 0.3.0 installed over
it:

| Criterion | Result |
|---|---|
| The update branch | `nstack/update-v0.3.0`, `main` untouched, `_commit: v0.3.0`, no conflict |
| 4. Fix and adaptation merged | Exercised on published versions for the first time: the README sentence and the `AGENTS.md` section kept; `CODEOWNERS` gains `*` above the team's module line |
| 7. Modules untouched | `modules/catalog/` unchanged |
| What arrives | `pull-request.yml`, `playbooks/framing.md`, `discovery.md`, `verification.md`, `docs/project/` and its templates; 29 files changed |
| M10 | `nstack fitness` fails on `catalog` — "module.user_facing must be true or false" — until the team declares it; then fitness, hooks, history scan and `doctor` L1–L6 green; `pr-scope` over budget (840 lines, 30 files), accepted with the label |
| `pr-check` on the update pull request | **Fails T1**: the update touches `contracts` (criticality high) and the migrated module. The `out-of-cycle` label with its justification lifts K1; nothing lifts T1 |

**What it teaches.**

- **An update is not delivery work, but `pr-check` reads it as such** (D24). Every version
  that changes a file under the skeleton's `contracts/` will ask the update pull request
  for a test sheet, whose scenarios have no behaviour to observe. To decide before the
  pilot's first update, since PDR-0001's criterion needs one merged; a branch-name
  exemption is not an option — any agent can name a branch.
- **A breaking manifest rule reaches modules the update never touches**: the team migrates
  them in the update pull request, as at v0.2.0, and the failure names the rule and the
  question to answer.
