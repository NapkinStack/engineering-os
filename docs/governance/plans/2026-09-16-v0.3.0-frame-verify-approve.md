...
cycle), a new optional standard verb `e2e`. What cannot be checked goes to the kernel, two
...
| `src/napkinstack/fitness/plan.py` | | | | | create: C1–C6 | | |
...
| `skeleton/.nstack/skills.yaml` | | | | `verification` | | `framing` | |
...
| `skeleton/playbooks/verification.md`, `framing.md` | | | | create `verification` | | create `framing` | |
...
- [ ] **Step 3: Translate the test suite**
...
- [ ] **Step 6: Correct ADR-0003's observation**
...
- [ ] **Step 3: Accept a user in the engine**
...
- [ ] **Step 5: The template's question**
...
- [ ] **Step 6: The suite, red then green**
...
- [ ] **Step 7: Record the clarification in PDR-0001**
...
- [ ] **Step 3: The skeleton's first rule**
...
- [ ] **Step 3: The README's once-only steps**
...
- [ ] **Step 4: Check the kernel's budget and the English**
...
- [ ] The usual verification (§Verification), plus: a trial project from the working tree
...
- [ ] Pull request "M8a — Identity and owner", merge.
...
- [ ] **Step 3: The rule, the template, the flag**
...
- [ ] **Step 4: CI runs it and keeps the evidence**
...
- [ ] **Step 5: Run the suite and the workflow hooks**
...
- [ ] **Step 5: The checklist and the commands**
...
- [ ] The usual verification (§Verification), plus: in a trial project, a pull request body
      written to a file with a complete sheet on the head commit passes
      `nstack pr-check --body-file`.
- [ ] Pull request "M8b — Test sheet: the engine", label `over-budget` with its
      justification, merge.
...
- [ ] **Step 1: Write the check, and see it fail on purpose**
...
- [ ] **Step 1: The loop and the definition of done**
...
- [ ] **Step 2: Quality, platform, profile**
...
- [ ] **Step 3: Pointers from the playbooks and the contributors' guide**
...
- [ ] **Step 4: Run the suite and the links**
...
- [ ] The usual verification (§Verification). No tool name added outside
...
- [ ] Pull request "M8c — Test sheet: the rules", merge.
...
- [ ] **Step 4: The pull request template and the issue form**
...
### Task 3 — Commands in the READMEs, verification, pull request

- [ ] `README.md` commands table: `| nstack plan | The charter and the cycles: formats, one cycle at a time, closures |`,
      ...
      the fitness table `| nstack plan | C1-C6: charter, cycles, deliverables, closures |` and
      `nstack pr-check`'s rules `T1-T5, K1-K4`, tests `| tests/test_plan.py | Rules C: one failing case per rule |`;
      `skeleton/README.md.jinja` commands: `nstack plan   # the charter and the cycles (docs/project/)`
      and `nstack fitness`'s comment "manifests + boundaries + skills + plan".
- [ ] The usual verification (§Verification).
- [ ] Pull request "M8d — Cycles: the engine", label `over-budget` with its justification, merge.
...
- [ ] **Step 4: The project folder's README**
...
```markdown
**Posture.** By default you are the **author**: one deliverable, one module. Asked to frame
a project or a cycle, you are the **framer**; asked to verify a change you did not write,
the **verifier** — each loads its playbook (§4). You never verify or approve your own work.
```
...
passing; `wc -l skeleton/AGENTS.md` at most 221.
...
| **Posture** | What a participant does, and never does: framer, author, verifier, approver, decider, owner (`05-workflow.md` §10). |
...
- [ ] **Step 3: Decisions, platform, measurement**
...
- [ ] **Step 4: README and contributors' guide**
...
```
    ...
    D --> F["Framing with your agent<br/>charter + first cycle"]:::team
...
- [ ] **Step 5: Run the suite, the hooks, the diagrams**
...
- [ ] The usual verification (§Verification), plus: no tool, stack or business domain
      named in the framework's new text (the M8c search, extended to the diff of M8e).
- [ ] A **dry-run framing** on an invented, public subject — not the pilot's — in a trial
...
- [ ] Pull request "M8e — Framing: the rules", label `over-budget` if beyond the budget,
      merge.
...
- **Specification coverage.** ADR-0004: the App and its permissions (M8a README, M8f), no
  ...
  removal condition and the measurement (M8e §3, pilot). PDR-0002: levels (M8e), postures
