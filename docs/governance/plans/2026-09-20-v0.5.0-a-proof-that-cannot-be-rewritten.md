...
- [x] **Step 1: Write the failing test.** A module whose `compat` command calls a file of its
...
- [x] **Step 2: Run it, see it fail** — `uv run pytest platform/tests/test_compat.py -k base -v`.
      Expected: it returns 0, because the command runs in the head's tree and finds the rewritten
      script. That failure **is** D44.

- [x] **Step 3: Extract the base's tree once per run.** In `compat.py`, beside `_extract`:
...
- [x] **Step 4: Run the command there.** In `run()`, replace the temporary-directory block:
...
- [x] **Step 5: Run the test, see it pass.**

- [x] **Step 6: The neighbour must still pass** — a test where the proof is honest and the change
      is additive: V1 returns 0. Without it, "refuse everything" would satisfy step 5.

- [x] **Step 7: A module folder that does not exist at the base** — the command is already read
...
- [x] **Step 1: Write the failing test** — a proof that cannot run (`exit 127`) must not be
...
- [x] **Step 2: Run it, see it fail** — today's message says "a breaking change".

- [x] **Step 3: Rewrite the failure**, naming the command's home instead of repeating it:
...
- [x] **Step 4: Run the tests, see them pass**, including one asserting the command text appears
      once in the whole output.

- [x] **Step 5: The handbook follows** — `03-contracts.md` §4 says the proof runs in the base's
      tree and may read of the head only the document it judges.

### Task 3 — The verdict is never hidden (D46)

- [x] **Step 1: Write the failing test** — in `platform/tests/run.sh`, assert that
      `module-checks.yml`'s `compat` step carries a condition that survives an earlier failure.

- [x] **Step 2: Run it, see it fail.**

- [x] **Step 3: Add the condition**, with its reason in a comment:
...
- [x] **Step 4: Run `run.sh` in full**, with `GITHUB_ACTIONS=true GITHUB_WORKFLOW=Governance`.

- [x] **Step 5: Mutations** — three, each reverted: remove the base tree (the D44 test goes red),
      restore the old message (the D45 test goes red), remove the condition (the D46 test goes
      red).

- [x] **Step 6: Commit** — `v0.5.0a — The proof runs in the base's tree, and says what it knows`.
...
- [x] **Step 1: Write the failing tests** — `K1` on a project whose charter is accepted and whose
...
- [x] **Step 3: Split the message in two**, in `pull_request.py`:
...
- [x] **Step 4: `new-module`'s next steps follow** the same split, so the generator never sends
      a framed project back to framing.

- [x] **Step 5: `Every touched module passed` becomes `Every changed module passed`** in
      `module-checks.yml`, matching the word `nstack modules --changed-since` uses (D42's
      wording), with the assertion in `run.sh`.

- [x] **Step 6: Run the tests and `run.sh`**, then mutate each message back and watch the tests go
      red.

- [x] **Step 7: Commit** — `v0.5.0b — K1 names what is missing, and one word for one thing`.
...
- [x] **Step 1: Write the failing tests** — the three states, from PDR-0006's acceptance
      criteria:
...
- [x] **Step 3: Classify, then report.** Each checklist rule gets one of `OK`, `GAP`,
...
- [x] **Step 4: The output in three blocks** — what is enforced here, what nothing enforces here
...
- [x] **Step 5: The exit code** — 0 when no `GAP` and no `UNKNOWN`; `OUT_OF_REACH` never decides
      it. A test asserts an unguarded, fully-reachable repository exits 0 **and** prints
      *unguarded*.

- [x] **Step 6: `run.sh`'s doctor cases updated** — the existing ones assert the old wording; each
      becomes an assertion on the new state line, plus one new case for the exit code.

### Task 2 — What a new project reads

- [x] **Step 1: The skeleton's README** keeps the checklist word for word (the identity test in
...
- [x] **Step 2: `07-governance.md`** names the two states where it describes the checks.
- [x] **Step 3: Run `run.sh`** — the checklist identity test must still pass in both directions.
- [x] **Step 4: Commit** — `v0.5.0c — A repository says whether anything refuses`.
...
- [x] **Step 1: Write the failing tests**, the forge answered from a stub:
...
- [x] **Step 2: Run them, see them fail** — no module.

- [x] **Step 3: Write `landed.py`** — the commits of the span, minus the parentless ones and
...
- [x] **Step 4: Run the tests, see them pass**, then mutate: return a pull request for every
      commit and watch the first test go red.

- [x] **Step 5: Wire the verb** into `cli.py`, and into `JUDGES` so the run names the framework
      that judged it (PDR-0005).

### Task 2 — The workflow the project receives

- [x] **Step 1: Write the failing assertion in `run.sh`** — a generated project holds
      `.github/workflows/commits-on-main.yml`, its token is read-only, and it calls the verb
      with the push event's range.

- [x] **Step 2: Run it, see it fail.**

- [x] **Step 3: Write the workflow** — `on: push: branches: [main]`, `permissions: contents:
...
- [x] **Step 4: The README and the handbook** say what the record is and, in the same sentence,
      what it is not: it records, it does not refuse.

- [x] **Step 5: Run `run.sh` in full**, the hooks, actionlint and zizmor on the new workflow.

- [x] **Step 6: Commit** — `v0.5.0d — What nothing could refuse is recorded`.
...
- [x] **Step 1: Write the failing test** — an engine installed from a repository at a tag names
      the tag:
...
- [x] **Step 2: Run it, see it fail** — today the line carries the commit only.

- [x] **Step 3: Name the requested revision** when `direct_url.json` records one
...
- [x] **Step 4: Run the tests, see them pass**, with the branch case unchanged.

- [x] **Step 5: The two channels documented** — `README.md` gains the forge install pinned to a
...
- [x] **Step 6: `PRODUCT.md` §7** follows, and the release notes of this version name the second
      channel.

- [x] **Step 7: Run `run.sh`** — the first-line assertions of every judging command still hold.

- [x] **Step 8: Commit** — `v0.5.0e — The forge distributes what the registry publishes`.

---

## Release v0.5.0 — PR 6

- [x] Version pull request (`uv version --bump minor`), whose description carries the **release
...
- [x] **The maintainer's explicit consent**, then the annotated tag on the merge commit, pushed
      by the agent under its App; the maintainer approves the `pypi` deployment.
- [x] Verification on published artefacts, as at v0.4.0: the PyPI JSON; `pypi-attestations
...
- [x] `nstack update` from v0.4.0 on a clone of `tool-library`: the adaptations the release notes
      announce, and nothing else.

## Closure — PR 7

- [x] This plan ticked with what was observed; `workstreams.md` — D44 to D47 closed, the v0.5.0
      row done, M11 next; `PRODUCT.md` §7 and the README's status.
- [x] The validation log's entry: the update's cost, and the probes replaying D44 — a proof that
...
## Observed — from the plan to the published v0.5.0, 2026-09-20

| Step | Pull request | What differed from this plan |
|---|---|---|
| The plan | #73 | Written after three measurements rather than after a prototype: the base tree's cost, what `uv` records for a tag install, and how the forge answers for a merged commit and a direct push. The blocks that rested on those three needed no correction; the others were decided by their tests |
| a — D44, D45, D46 | #74 | Two cases the plan did not foresee, both kept: a module whose folder does not exist at the base is named rather than crashing, and a proof that cannot run at all (exit 127) is explicitly not called a breaking change |
| b — D47 | #75 | `pr-scope`'s `Modules touched` deliberately left alone: it counts modules changed **beyond their description** (D24), a different set from the one CI lists. Renaming it would have made two notions share one word — the defect the row is about |
| c — PDR-0006 | #76 | The plan named `07-governance.md` for the two states; the handbook never mentions the diagnosis, so they are introduced in the project's own README instead. No `test_doctor.py` was created either: `run.sh` already drives `doctor` against a fake forge with seven fixtures, and that is where the cases belong |
| d — the record | #77 | Two real bugs, caught by the tests and not by rereading: `git rev-parse` echoes an unresolved argument on stdout, so the first commit of a repository was being recorded; and an `and` expression swallowed the `-1` limiting a first push to its tip. The handbook's §7 gained a paragraph the plan had not asked for: what remains when the forge can enforce nothing |
| e — two channels | #78 | Nothing |
| Version | #79 | Nothing |
| Tag and publication | `v0.5.0` on `309fd21` | The maintainer's consent, then the `pypi` deployment approved by the maintainer |

**The publication, verified on published artefacts:**

| Verification | Result |
|---|---|
| PyPI | `napkinstack` 0.5.0, MIT, Python ≥ 3.12, a wheel and a source archive |
| Attestations | OK for both files, verified `--offline` from the downloaded files and the provenance served by PyPI's integrity API — the sandbox's proxy refuses Sigstore's TUF CDN. Publisher `release.yml`, environment `pypi`, certificates naming `refs/tags/v0.5.0` |
| A fresh install | `uv tool install napkinstack==0.5.0` in isolated directories: `nstack 0.5.0` |
| `nstack init` from the tag | `_commit: v0.5.0`; the generated project carries `commits-on-main.yml` beside the three other workflows; `fitness` compliant, `plan` "not framed yet"; no accented letter |
| The record, on a project with no token | *"no token: how each commit arrived is not verified"*, and the run passes — nothing claimed |
| `doctor` on the generated project | L1 and L7 OK on the published engine; with no token, *"This repository: not verified — what cannot be read is never reported as guarded"* |
| The forge channel | `uvx --from git+…@v0.5.0 nstack init` resolved the tag to `309fd21` and recorded `_commit: v0.5.0` — a project identical to one created from the registry |
| `nstack update` from v0.4.0 | A clone of `tool-library`: **no conflict, no adaptation asked** — 6 files, 84 lines, `fitness` and `pr-check` compliant with no label and no sheet |
| V1, on the real project | The same clone, `catalog-api` v1 broken consistently: `FAIL [V1] … did not prove the change compatible (exit 1)`, with oasdiff's two `response-required-property-removed` errors visible above the verdict rather than buried under a repeated command |
