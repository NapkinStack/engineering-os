...
> M2a, M2b, M3, M3.1, M3.2, M4, M6, M6.1 et M5 (faits), puis le projet pilote.
...
**Spécification :** [PDR-0001](../../pdr/0001-create-a-project-and-receive-updates.md)
(accepté), [ADR-0001](../../adr/0001-adopt-copier-to-generate-and-update-projects.md)
(accepté), [ADR-0002](../../adr/0002-distribute-napkinstack-on-pypi.md) (accepté).
...
flowchart LR
    ...
    M61 --> M5["M5<br/>publication v0.1.0"]:::fait
...
| **M6 — Documentation produit** | README réécrit avec les schémas du PDR ; CONTRIBUTING ; `docs/os/09-platform.md` ; C1, C2, C4, C5 replanifiés | PDR-0001 (impacts) | Relecture |
...
- Modifier : `.github/workflows/governance.yml`, `.github/dependabot.yml`,
  ...
  `docs/governance/workstreams.md` (DoD, D18, D21, C1)
    ...
    for playbook in sorted(playbook_dir.glob("*.md")):
        if playbook.name not in declared:
            failures.append(
                ...
                f"playbook s'il ne sert plus (docs/os/10-measurement.md §6)."
            )
...
git grep -n -E "platform/(fitness|sync_skills|scaffold|templates|tests)|\bmake (bootstrap|check|test|fitness|skills|skills-check|scaffold|ci)\b|requirements-dev\.txt" \
  -- ':!docs/governance/workstreams.md' ':!docs/governance/plans/' ':!docs/adr/' \
...
```
  ...
  - `docs/governance/workstreams.md` : DoD `bash platform/tests/run.sh` →
...
- Modifier : `src/napkinstack/skills.py`, `platform/tests/run.sh`, `pyproject.toml`,
  ...
  `docs/governance/workstreams.md`, `docs/governance/automation-backlog.md`,
  `docs/governance/reviews.md`.
...
- [x] **Étape 7 : contenu du squelette.**
  ...
  - `skeleton/docs/os/09-platform.md` : ligne 182, `platform/tooling-profile.md` →
...
> Suivi : [`docs/governance/workstreams.md`](docs/governance/workstreams.md).
...
- Chantiers et Definition of Done : [`docs/governance/workstreams.md`](docs/governance/workstreams.md).
...
git grep -n -E "(^|[^/])(docs/os/|playbooks/)|platform/(skills\.yaml|tooling-profile\.md)" -- . \
  ...
  ':!docs/governance/plans/' ':!docs/governance/workstreams.md' \
...
- [x] **Étape 3 : corriger chaque occurrence.**
  - `.github/CODEOWNERS` : lignes `/AGENTS.md`, `/playbooks/`, `/docs/os/`, `/contracts/`
    remplacées par `/skeleton/` et `/copier.yml` ; en-tête, `docs/os/07-governance.md` →
    `skeleton/docs/os/07-governance.md`.
  - `.github/workflows/governance.yml` : en-tête, `docs/os/07-governance.md` →
    `skeleton/docs/os/07-governance.md`.
  - `PRODUCT.md` §1 : première ligne du tableau → `` `skeleton/AGENTS.md`, `skeleton/playbooks/`,
    ...
    `` le kernel `skeleton/AGENTS.md` `` ; §6 → `` `skeleton/docs/os/07-governance.md` §3 ``.
    *Chemins complets à l'exécution : l'inventaire ne distingue pas « `playbooks/` sous
    `skeleton/` » d'une référence morte.*
  - `SECURITY.md` : `playbooks/security.md` → `skeleton/playbooks/security.md` ; `AGENTS.md`
    §5 → `skeleton/AGENTS.md` §5.
  - `platform/README.md` : lignes `skills.yaml` et `tooling-profile.md` du tableau
    supprimées ; S1 → `` `.nstack/skills.yaml` `` ; `docs/os/07-governance.md` →
    `skeleton/docs/os/07-governance.md`.
  - `docs/adr/README.md`, `docs/pdr/README.md`, `docs/governance/automation-backlog.md`,
    `docs/governance/reviews.md` : `docs/os/` → `skeleton/docs/os/`.
  - `docs/governance/workstreams.md` : nœud `RP` → `Moteur v0.1.0<br/>M1 fait, M2a en cours` ;
...
- Modifier : `src/napkinstack/cli.py`, `platform/tests/run.sh`, `skeleton/README.md.jinja`,
  `README.md`, `platform/README.md`, `docs/governance/workstreams.md`, ce plan.
...
sed -i '1s/.*/# Sécurité — titre v0.3/' "$TPL/skeleton/playbooks/security.md"
...
B="$GN/projet-b"
projet_v01 "$B"
sed -i '1s/.*/# Sécurité — adaptation locale/' "$B/playbooks/security.md"
...
- [x] **Étape 4.** `docs/governance/workstreams.md` : nœud `RP` →
...
pas de revue des CODEOWNERS (ADR-0004, identité de l'agent, à faire), pas de label
`cross-module`.
...
- Modifier : `src/napkinstack/cli.py`, `src/napkinstack/project.py` (checklist affichée par
  ...
  `docs/governance/workstreams.md`, ce plan.
...
- [x] Labels `cross-module` et `hors-budget` : ils rendent les exceptions visibles **et
  comptables** (`docs/os/10-measurement.md` §3)
...
- [x] **Étape 2.** `docs/governance/workstreams.md` : nœud `RP` →
...
`docs/pdr/0001-create-a-project-and-receive-updates.md`, `docs/pdr/README.md`,
`docs/governance/workstreams.md`, ce plan.
...
- [x] **Étape 4 : suivi.** `docs/pdr/README.md` : statut « Accepté, précisé (dépôts privés) » ;
  `docs/governance/workstreams.md` : nœud `RP` → `Moteur v0.1.0<br/>M3 fait, M3.1 en cours`,
...
|---|---|---|
| Aucune page ne dit que NapkinStack est un framework ; « ni framework » (sous-entendu applicatif) se lit « pas un framework » | `PRODUCT.md` §2, README du squelette, `docs/os/00-overview.md` §5 | Positionnement explicite : **framework de travail** (modèle Django, Rails), pas framework applicatif ; schéma légendé du produit dans `PRODUCT.md` §1 |
| Vocabulaire flottant : « OS », « starter », « template », « socle copié » | `PRODUCT.md`, README du squelette, `docs/os/README.md`, `09-platform.md` §3, §7 | Glossaire : framework, moteur, squelette, OS (la méthode, sans marque), socle ; « starter » → « squelette » |
...
| Statuts de défauts à jour | `workstreams.md` : D6, D19, D22 | D6 → M3.2 ; D19 et D22 → M4 |
| Description du dépôt vide sur GitHub | Réglages du dépôt | Action humaine : le jeton de l'agent n'écrit pas les réglages |

**Hors de ce chantier :** la réécriture de fond de `docs/os/09-platform.md` (arborescence
...
printf 'Voir `docs/03-contracts.md` §4.\n' > "$RM/regle.md"
...
- [x] **Étape 4.** Manuel : `00-overview.md` §5 « framework applicatif » et glossaire
  (squelette, moteur, sans marque) ; `README.md` du manuel : public « création du projet » et
  arborescence réelle ; `09-platform.md` : « starter » → « squelette ».
- [x] **Étape 5.** `platform/README.md` : en-tête en double supprimé ; `workstreams.md` : nœud
  `RP`, D6, D19, D22.
...
`platform/README.md`, `docs/governance/workstreams.md`.
...
# (docs/os/09-platform.md §2). `nstack <verbe> <module>` les exécute depuis ce dossier.
...
> Obligatoire pour criticality={criticite} (docs/os/08-quality.md §7).
...
def nouveau(root: Path, nom: str, owner: str, criticite: str) -> int:
    ...
    if not EQUIPE.fullmatch(owner):
        print(f"ÉCHEC [new-module] owner '{owner}' invalide : une équipe GitHub organisation/équipe, "
              "par exemple acme/facturation (CODEOWNERS, docs/os/07-governance.md §7).")
        return 1
    ...
    for cible in cibles:
        ...
        if not commande:
            ...
            print(f"ÉCHEC [{verbe}] module '{cible}' : commands.{verbe} non déclarée dans {manifest}.\n"
                  "      Action : y déclarer la commande de la stack du module (docs/os/09-platform.md §2).")
            return 1
...
- [x] **Étape 3 : documentation.**
  ...
  - `docs/governance/workstreams.md` : nœud `RP` → `M3.2 fait, M4 en cours` ; C1 → « Traité par M1
    et M4 » ; D19 et D22 → « M4 ».
- [x] **Étape 4 : vérifier et commiter.** Inventaire `git grep -n -E '\bmake\b|Makefile' -- skeleton/ src/ README.md platform/`
  (seule la mention générique de `09-platform.md` reste) ; hooks ; tests ;
...
> `docs/os/09-platform.md`, replanification de C2 et C4 (C1 et C5 sont traités).
...
|---|---|---|
| Le chapitre plateforme décrit l'ancien modèle : un module `platform/` généré avec fitness functions et gabarits, verbes « câblés », squelette de contrat v1, rulesets fournis | `skeleton/docs/os/09-platform.md`, `02-modules.md` §7 | Réécriture : plateforme à deux étages (moteur versionné, socle du projet), arborescence réelle, création de module exacte, schémas légendés ; sans marque (P7) |
| Contrôles annoncés « rouge » qui n'existent pas (D15) | index et modèle des ADR (racine et squelette), `03-contracts.md` étape 4, playbook sécurité | Libellés « revue, à automatiser » ; lignes au backlog d'automatisation |
...
| C2, C4, D7, D9, D23 | `workstreams.md` | Après le projet pilote, sur preuve (`PRODUCT.md` §6) |

### Tâche 1 — Chapitre plateforme

- [x] **Étape 1.** Réécrire `skeleton/docs/os/09-platform.md` :
...
- [x] **Étape 1.** Index des ADR (racine et squelette) : colonne « Contrôle » → « Revue, à
  ...
  revue ; à automatiser » ; `03-contracts.md` étape 4 : « Revue : consommateurs v1 = 0, à
...
- [x] **Étape 2.** Ces contrôles relèvent du moteur : lignes ajoutées au backlog
  d'automatisation de la racine (`docs/governance/automation-backlog.md`) — sections
...
- [x] **Étape 2.** `workstreams.md` : en-têtes de C2 et C4 → « Après le projet pilote, si le
...
- [x] `docs/governance/workstreams.md` : D4, D5, D8 → « M6.1 » traités ; nœud `RP` → « M6 fait,
...
- [x] `LICENSE` : texte MIT intégral, `Copyright (c) 2026 NapkinStack`.
- [x] `pyproject.toml` : `version = "0.1.0"`, `license = "MIT"`, `license-files = ["LICENSE"]`,
  `readme = "README.md"`, `[project.urls]` (Homepage, Source, Issues vers
  `https://github.com/NapkinStack/engineering-os`) ; `uv lock`.
- [x] Vérifier : `uv build` puis métadonnées du wheel (licence, version) ; tests verts.
...
- [x] **Étape 1 : test qui échoue.** Dans `platform/tests/run.sh`, avant les tests des hooks :
...
- [x] **Étape 2 : voir échouer** (`release.yml` absent), **implémenter** :
      ...
      - name: Tag et version identiques
        run: |
          ...
          if [ "${GITHUB_REF_NAME#v}" != "$VERSION" ]; then
            echo "::error::Tag $GITHUB_REF_NAME et version $VERSION de pyproject.toml différents (ADR-0002)." \
              "Action : aligner la version par une PR, puis poser le tag v$VERSION."
            exit 1
          fi
...
- [x] **Étape 3 : voir passer.** Tests verts ; hooks (zizmor, actionlint) verts sur le workflow.

### Tâche 3 — Documentation

- [x] `CONTRIBUTING.md` : section « Publier une version » avec le schéma légendé du circuit
  (PR de version, tag, construction, approbation, publication) et les commandes
  (`uv version --bump`, tag).
- [x] `README.md` : installation `uv tool install napkinstack --with-executables-from pre-commit`
  (dès la v0.1.0 publiée) ; ligne « Licence : MIT ».
- [x] `workstreams.md` : nœud `RP` → « M6.1 fait, M5 en cours ».

### Tâche 4 — PR et merge

- [x] Vérifications habituelles ; PR « M5 — Publication v0.1.0 » ; merge après CI verte et rejeu.

### Tâche 5 — Publication (actions humaines guidées)

- [x] **Mainteneur** : Settings → Environments → `pypi` ; relecteur obligatoire
  `napkinstack-admin` ; déploiements limités aux tags `v*`. **Agent** : vérifie par l'API.
- [x] **Agent**, avec l'accord du mainteneur : tag `v0.1.0` sur le commit de merge, poussé par
  l'alias SSH. **Mainteneur** : approuve le déploiement dans GitHub.
- [x] **Agent** : vérifie `uv tool install napkinstack==0.1.0` sur un poste neuf (dossiers
  ...
  `PRODUCT.md` §7 à jour.
  *Écart constaté à l'exécution : l'environnement `pypi` n'avait d'abord aucune règle ; le tag
  n'a été poussé qu'après relecture par l'API (relecteur obligatoire, tags `v*`). Tag annoté,
  comme le recommande git pour une version ; `CONTRIBUTING.md` aligné.*

**Constaté le 2026-09-15** (exécution 35027054928, tag `v0.1.0` sur `85a1fa8`) :

| Vérification | Résultat |
|---|---|
| PyPI | `napkinstack` 0.1.0, licence MIT, wheel et archive source |
| API d'intégrité de PyPI | Une attestation par fichier : éditeur GitHub, `NapkinStack/engineering-os`, `release.yml`, environnement `pypi` |
| Secrets | Aucun référencé par `release.yml` ; l'action PyPA ne reçoit aucun mot de passe |
| Poste neuf (`UV_TOOL_DIR`, `UV_TOOL_BIN_DIR`, `UV_CACHE_DIR` isolés, `UV_NO_CONFIG`) | `uv tool install napkinstack==0.1.0 --with-executables-from pre-commit` : `nstack 0.1.0` et `pre-commit` |
| `nstack init` depuis GitHub | `_commit: v0.1.0`, commit « Création du projet, NapkinStack v0.1.0 » |
| Projet généré, avec cette installation : sa CI rejouée | `nstack fitness`, hooks, `gitleaks-historique`, `pr-scope`, `doctor` L1 et L2 verts ; `new-module` crée le module et son CODEOWNERS, `nstack check` échoue explicitement tant que la commande n'est pas déclarée |

**Non vérifiable dans ce chantier :** la CI d'un projet généré exécutée sur GitHub (projet
pilote) ; `nstack update` d'une version publiée vers la suivante (prochaine publication).

**PR de bilan :** ADR-0002 accepté et index ; README (état, installation), `PRODUCT.md` §7,
`workstreams.md` (ADR-0002, moteur publié, nœud du projet pilote), cette section.
