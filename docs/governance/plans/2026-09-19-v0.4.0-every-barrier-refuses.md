...
- [x] **Agent**: `uvx --from napkinstack==0.4.0 nstack update` in a clone of `tool-library`;
      ...
      agent merges.
- [x] **Maintainer**: the ruleset of `main` requires `Module checks`; `nstack doctor` with a
      short-lived token of their own: G1 to G13 and L1 to L7 compliant.
- [x] **Agent**: `contracts` declares `commands.compat` — oasdiff, pinned, installed by its
      `bootstrap` — in a pull request of its own, since V1 reads the proof as merged.
      **Maintainer**: approves; the agent merges.
- [x] **Agent**: the modules' `uv.lock` committed (D34 #10), in a pull request of its own.
- [x] **The probes, replayed** — each a pull request titled `Probe P<n> — <what>`, never
...
- [x] **Agent**: the log's closing comment — each probe's outcome against M9's, the update's
      adaptations and their cost, every sync event.

## Closure — PR 9

- [x] This plan ticked, with what was observed; `workstreams.md` — M10 done, the rows closed,
      D34's destinations; `PRODUCT.md` §7; the README's status; the automation backlog.
- [x] **PDR-0002, clarified** (D34 #1): the framing's criterion is stated in effective work,
...
- [x] **The maintainer, decider**: the exit decision — **go**: the organisation moves to
      GitHub Team and the private pilot starts; or **fix first**, the defects to close.
      **Fix first**, 2026-09-19: the fix before the pilot (D44 to D47). Shipped as v0.5.0,
      with PDR-0006 and ADR-0002's clarification, by the maintainer's choice of 2026-09-20.
...
## Observed — M10h and the closure, 2026-09-19

On `NapkinStack/tool-library`, each step logged as it happened in its issue 1.

| Step | Pull request | What differed from this plan |
|---|---|---|
| The update, with the contracts module's `check` and `test` | tool-library #31 | A test sheet and two labels, `out-of-cycle` and `over-budget`, where the plan said none: the update alone needs neither (measured before the release), but its adaptation adds tests to a high-criticality module. The sheet's first run found a real gap — a 404 example whose body said 400 |
| `Module checks` required; `nstack doctor` | the maintainer | Nothing: L1 to L7 and G1 to G13 compliant, `Module checks` among G4's checks |
| `contracts` declares `compat` | tool-library #32 | Self-contained rather than installed by `bootstrap`: V1 runs the merged command in the pull request's tree, so a file it called could be rewritten by the change it judges — D44 |
| The modules' `uv.lock` | tool-library #33 | Four verifier runs on four heads, each finding something real: `uv run --locked --with` resolves outside the lock; CI ran Python 3.14 where the charter says 3.13; verbs sharing one environment passed on another group's packages; two traps described with the wrong mechanism |
| The probes | tool-library #34 to #41 | P4c added: M9's P4, replayed as written, is now refused by the contracts module's `test` (#31) before V1 runs. P4c, the same break made consistently, is refused by V1 alone |
| The closing comment, the exit decision | issue 1 | **Fix first**: V1 can be walked around by a proof that calls a file (D44) |

**The probes, against M9** — eight of eight did what v0.4.0 promises:

| # | The probe | On v0.3.1 (M9) | On v0.4.0 | Refused by |
|---|---|---|---|---|
| P2 | `loans` imports `catalog`'s code behind `sys.path.insert` | accepted | refused, #34 | B2, twice |
| P3 | `loans` reads the contract with `consumes` emptied | accepted | refused, #35 | B1 |
| P4 | `v1` broken in place, the examples left behind | caught by luck, by the producer's `e2e` | refused, #38 | the contracts module's `test` and `catalog`'s, before V1 runs |
| P4c | the same break, examples and producer's tests included | not run | refused, #39 | V1 alone; the consumer broken, no job of its own ran |
| P4b | an optional property added to `v1` | — | passed, #40 | V1: *proven compatible* |
| P12 | a sheet signed by the session that wrote the change | accepted | refused, #37 | T2 |
| P13 | a module test that fails | mergeable once approved | blocked, #41 | `Module checks`, a required check |
| P14 | `nstack new-module --user-facing`, alone | `check` and `test` exit 1 | passed, #36 | — |

**What the update cost**: the update itself, nothing — Copier's merge, no conflict. Its
adaptations: `check`, `test` and `compat` on the contracts module. The maintainer: three
approvals, one required check, one `doctor`. No sync event.

**Found, placed in the register**: D44 to D47 in v0.5.0, the pilot's precondition; D48 after
the pilot, if its modules lock with `uv`; D49 with the verifier's own App (ADR-0004). Not
NapkinStack's: the validation project's contract test runs `uvx --with`, unlocked; Copier's
*"Git >= 2.24"* warning on Git 2.55, upstream and harmless.
