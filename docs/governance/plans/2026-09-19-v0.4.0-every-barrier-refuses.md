...
- [ ] **Step 3: The description, what a module holds, and M7**
...
- [ ] **Step 4: Run them, then all the unit tests**
...
- [ ] **Step 2: Run them — `run.sh` first, since it runs pytest before its own blocks**
...
- [ ] **Step 3: The verbs, the next steps, the templates**
...
- [ ] `skeleton/docs/os/09-platform.md` §2 — the sentence "`check` and `test` are mandatory,
      ...
      other file."
- [ ] Same file, §4's diagram: `C3["check and test commands<br/>to be declared, failing"]`
...
- [ ] `skeleton/README.md.jinja`, "What is not provided": "A new module therefore declares
...
- [ ] `workstreams.md`: the D33 row, "v0.4.0: M10a".
- [ ] Verification (§ *Verification — every pull request*), then the pull request
...
- [ ] **Step 3: `pr-scope` in Python, and the list of modules**
...
- [ ] **Step 1: The failing test — G4 read in both directions**
...
- [ ] **Step 3: The workflow, the checklist**
...
- [ ] `skeleton/docs/os/02-modules.md` §7, after the rule's paragraph: "A module is touched
...
- [ ] `platform/README.md`: `pr-scope`'s row "P1-P2: one PR = one module — changed beyond
      its description, `contracts/` included — review budget"; `nstack modules` in the
      commands; the file table names `fitness/pr_scope.py`.
- [ ] `README.md`'s command table and `skeleton/README.md.jinja`'s "The commands": `nstack
      modules [--changed-since <base>]` — the project's modules, or those a change touches.
- [ ] `docs/governance/automation-backlog.md`: the `pr_scope.sh` reference becomes
      `fitness/pr_scope.py`.
- [ ] `workstreams.md`: D35 "v0.4.0: M10b"; D34 #2 and #7 done; D7 "M10b: one definition
...
- [ ] **Step 2: Run them against today's `boundaries.py`**
...
- [ ] **Step 3: Mutations** (`PYTHONDONTWRITEBYTECODE=1`, each reverted after its run)
...
- [ ] For P2 and P3: `gh-agent pr diff 22 --repo NapkinStack/tool-library > p2.diff` (then 23),
      ...
      declaring it".
- [ ] The outputs go in the pull request's VERIFIED block.

### Task 4 — The handbook, the templates, the pull request

- [ ] `skeleton/docs/os/02-modules.md` §5: "An import towards an undeclared module fails in
...
- [ ] Same file, §8, at the end of "Allowed but worth watching": "They live outside the
      module folders: a module never imports another module's code, and declaring its
      contract in `consumes` does not change that."
- [ ] `src/napkinstack/templates/module/MANIFEST.yaml`: `# Contracts PROVIDED by this
...
- [ ] `platform/README.md`: `nstack boundaries` — "B1-B7: the contracts each module reads
...
- [ ] `docs/governance/automation-backlog.md`: "Consumers of a removed version = 0" moves to
      the automated rules — `nstack boundaries` B6, 2026-MM-DD; "Declared graph = real graph"
      gains the same date, and "contracts included".
- [ ] `workstreams.md`: D29 and D34 #6, "v0.4.0: M10c".
- [ ] Verification; the pull request `M10c — D29: the real graph is the contracts a module
...
- [ ] **Step 1**: `module-checks.yml` — the `discover` job outputs its base:
```yaml
    outputs:
      modules: ${{ steps.list.outputs.modules }}
      any: ${{ steps.list.outputs.any }}
...
```
- [ ] **Step 2**: `skeleton/contracts/MANIFEST.yaml.jinja`, the comment above `commands`:
```yaml
# Declared with the first contract: check and test are required once this folder holds one
...
```
- [ ] **Step 3**: `uv run bash platform/tests/run.sh` — `Platform tests: OK`; the generated
      project's actionlint and zizmor read the step.
- [ ] **Step 4: Commit** — `M10d — The module checks run V1 on the module holding a changed version`.

### Task 4 — Replayed on the project that exposed it, with an established comparator

- [ ] `tool-library` cloned (Task 3 of M10c); P4's diff (`gh-agent pr diff 24`) applied and
...
- [ ] oasdiff 1.32.1, its release archive downloaded to a scratch folder
...
- [ ] The same base, an optional property `condition` added to `Tool` instead: `Contract
      version catalog-api v1 (consumed by loans): proven compatible.`
- [ ] The outputs in the pull request's VERIFIED block.

### Task 5 — The handbook, the profile, the pull request

- [ ] `skeleton/docs/os/03-contracts.md` §3, after "This is the first question to ask, and
...
- [ ] Same file, §4, "The two guardrails" become three: "**③ A version someone relies on
...
- [ ] `skeleton/docs/os/09-platform.md` §2, the verbs table gains
...
- [ ] `skeleton/contracts/README.md`, "Evolving a contract": **Additive** gains "— and, once
...
- [ ] `skeleton/docs/tooling-profile.md`: a row
      `| Contract compatibility | Proving a change to a version someone relies on compatible | | |`.
- [ ] `skeleton/README.md.jinja`, "What is left to wire": "The contract format chosen, and
      the contract tests that go with it" gains "and the comparator behind `commands.compat`".
- [ ] `platform/README.md`: `nstack compat`'s row, "V1: a version consumed or stable changes
...
- [ ] `workstreams.md`: D30, "v0.4.0: M10d".
- [ ] Verification; the pull request `M10d — D30: a contract version someone relies on
...
- [ ] **Step 2: Run the tests, then the suite**
...
- [ ] `skeleton/AGENTS.md` §0, **Identity** — after "and you never approve a pull request
...
```
- [ ] Same file, §4, after the loading order's block (D34 #8): "One session, one working tree:
...
- [ ] `skeleton/.github/pull_request_template.md`: `Verifier: <@handle, or session and the
      agent session's identifier>`.
- [ ] `skeleton/playbooks/verification.md`, after "The session that wrote the change cannot
      ...
      match."
- [ ] `skeleton/docs/os/05-workflow.md` §7, the bullet **Run by a verifier who is not the
...
- [ ] ADR-0004, a section before *Rejected alternatives*:
```
...
```
- [ ] `workstreams.md`: D32 and D34 #8, "v0.4.0: M10e".
- [ ] Verification; the pull request `M10e — D32: a verifier who is not an author`.
...
- [ ] **Step 1**: `doctor.py` — the docstring's G line:
```
...
```
- [ ] **Step 2**: `skeleton/README.md.jinja`, after "Code owner review required":
```
...
select them.
```
- [ ] **Step 3**: `uv run bash platform/tests/run.sh` — `Platform tests: OK`.
- [ ] **Step 4: Commit** — `M10f — doctor G13: an approval is dismissed when new commits arrive (D31); G12 and G4 name the right action (D25, D26)`.

### Task 3 — ADR-0004, the pull request

- [ ] ADR-0004, a section beside M10e's:
```
...
```
- [ ] `workstreams.md`: D31, D25, D26 "v0.4.0: M10f"; the Step 0 table gains "Ruleset `main`:
      stale approvals dismissed on push — verified through the API (2026-09-18)".
- [ ] Verification; the pull request `M10f — D31: an approval covers what it read`.
...
```
- [ ] **Step 2**: `uv run pytest -q platform/tests` — all pass.
- [ ] **Step 3: Commit** — `M10g — Every judging run names the framework that judges; doctor never calls an unpublished one compliant`.
...
- [ ] **Step 2: The script, and the five steps that use it**
...
- [ ] **Step 4: Commit** — `M10g — CI installs the framework the project records: PyPI, or the repository at a commit, announced (PDR-0005)`.

### Task 4 — The contributor's loop, written down

- [ ] `CONTRIBUTING.md`, after "A workstream's journey":
````
...
````
- [ ] `skeleton/README.md.jinja`, "Receiving a new version of NapkinStack", a closing
...
- [ ] PDR-0005's status: "Accepted (2026-09-18) … implemented in v0.4.0 (M10g)".
      `platform/README.md`: `doctor`'s rules L1 to L7, and the file table's rows for
      `provenance.py` and `tests/test_provenance.py`.
- [ ] Verification; the pull request `M10g — PDR-0005: the contributor's loop, and never judged
...
- [ ] Version pull request: `uv version --bump minor` (0.3.1 → 0.4.0), approved, merged. Its
...
- [ ] **The maintainer's explicit consent**, then the annotated tag `v0.4.0` on the merge
      commit, pushed by the agent under its App; the maintainer approves the `pypi`
      deployment.
- [ ] Verification on published artefacts, as at v0.3.0 and v0.3.1: the PyPI JSON;
...
- [ ] `nstack update` from v0.3.1 to v0.4.0 on a project adapted like `tool-library` — a
